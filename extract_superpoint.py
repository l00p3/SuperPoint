...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--H", type=int, default=480, help="Input image height (default: 120)."
    )
    parser.add_argument(
        "--W", type=int, default=640, help="Input image width (default:160)."
    )
    parser.add_argument(
        "--display_scale",
        type=int,
    ...
    while True:
        ...
        for pt, dsc in zip(pts.T, desc.T):
            a = np.concatenate((pt[:2], dsc))
            output.append(np.concatenate((pt[:2], dsc)))
        output = np.array(output)
        with open(output_folder + filename + ".dat", "wb") as f:
            np.save(f, output)
